	...
	case "game-win-ratio":
		...
		case "player":
			...
			sqlString = `
				...
				SELECT
					GP.PLAY_COUNT,
					COALESCE(GW.WIN_COUNT, 0) AS WIN_COUNT,
					COALESCE((GW.WIN_COUNT * 1.0) / (GP.PLAY_COUNT * 1.0), 0.0) AS WIN_RATIO,
					U.NAME AS NAME
				FROM USER AS U
...
func GetAchievementsUser(userId uuid.UUID) ([]Achievement, error) {
