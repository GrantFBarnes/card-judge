CREATE EVENT IF NOT EXISTS EVT_CLEAN_BAD_RESPONSE_CARDS
        ...
        WHERE ID IN (
            ...
            FROM (
                    ...
                    LEFT JOIN (
                        ...
                        FROM (
                            SELECT
                                PLAYER_CARD_ID,
                                CREATED_ON_DATE,
                                RANK() OVER (PARTITION BY PLAYER_CARD_ID ORDER BY CREATED_ON_DATE DESC) AS PLAY_ORDER
                ...
                WHERE LASTPLAYED.LAST_PLAYED_DATE IS NULL -- NEVER PLAYED
                    OR LASTPLAYED.LAST_PLAYED_DATE < LD.CREATED_ON_DATE -- DISCARDS SINCE LAST PLAYED
                GROUP BY LD.CARD_ID
            ) AS BADCARDS
