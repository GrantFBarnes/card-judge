...
CREATE TRIGGER LOBBY_USER_SET_CHANGED_ON_DATE
BEFORE UPDATE ON LOBBY_USER
FOR EACH ROW
SET NEW.CHANGED_ON_DATE = CURRENT_TIMESTAMP();

CREATE TABLE LOBBY_USER_CARD
(
    ID CHAR(36) NOT NULL DEFAULT UUID(),
    CREATED_ON_DATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    CHANGED_ON_DATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),

    LOBBY_USER_ID CHAR(36) NOT NULL,
    LOBBY_ID CHAR(36) NOT NULL,
    USER_ID CHAR(36) NOT NULL,
    CARD_ID CHAR(36) NOT NULL,

    PRIMARY KEY (ID),
    FOREIGN KEY (LOBBY_USER_ID) REFERENCES LOBBY_USER (ID) ON DELETE CASCADE,
    FOREIGN KEY (LOBBY_ID) REFERENCES LOBBY (ID) ON DELETE CASCADE,
    FOREIGN KEY (USER_ID) REFERENCES USER (ID) ON DELETE CASCADE,
    FOREIGN KEY (CARD_ID) REFERENCES CARD (ID) ON DELETE CASCADE,
    CONSTRAINT LOBBY_USER_CARD_UNIQUE UNIQUE (LOBBY_USER_ID, CARD_ID)
);

CREATE TRIGGER LOBBY_USER_CARD_SET_CHANGED_ON_DATE
BEFORE UPDATE ON LOBBY_USER_CARD
FOR EACH ROW
SET NEW.CHANGED_ON_DATE = CURRENT_TIMESTAMP();

CREATE TABLE LOBBY_USER_PLAY
(
    ID CHAR(36) NOT NULL DEFAULT UUID(),
    CREATED_ON_DATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    CHANGED_ON_DATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    ...
    CONSTRAINT LOBBY_USER_UNIQUE UNIQUE (LOBBY_USER_ID)
);

CREATE TRIGGER LOBBY_USER_PLAY_SET_CHANGED_ON_DATE
BEFORE UPDATE ON LOBBY_USER_PLAY
FOR EACH ROW
SET NEW.CHANGED_ON_DATE = CURRENT_TIMESTAMP();
