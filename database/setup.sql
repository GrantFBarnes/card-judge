...
(
    ...
    SURPRISE_CARD_LIMIT INT NOT NULL DEFAULT -1,
    WILD_CARD_LIMIT INT NOT NULL DEFAULT 0,
...
DELIMITER //
CREATE PROCEDURE SP_SET_SURPRISE_CARD_LIMIT (
    IN VAR_LOBBY_ID UUID,
    IN VAR_NEW_SURPRISE_CARD_LIMIT INT
)
BEGIN
    DECLARE VAR_OLD_SURPRISE_CARD_LIMIT INT;

    SELECT SURPRISE_CARD_LIMIT
    INTO VAR_OLD_SURPRISE_CARD_LIMIT
    FROM LOBBY
    WHERE ID = VAR_LOBBY_ID;
    
    UPDATE LOBBY
    SET SURPRISE_CARD_LIMIT = VAR_NEW_SURPRISE_CARD_LIMIT
    WHERE ID = VAR_LOBBY_ID;
    
    UPDATE PLAYER 
    SET SURPRISE_CARDS_REMAINING = 
		CASE 
			WHEN VAR_NEW_SURPRISE_CARD_LIMIT = -1 THEN -1
			WHEN VAR_NEW_SURPRISE_CARD_LIMIT = 0 THEN 0
			WHEN VAR_OLD_SURPRISE_CARD_LIMIT = -1 THEN VAR_NEW_SURPRISE_CARD_LIMIT
            WHEN VAR_OLD_SURPRISE_CARD_LIMIT = 0 THEN VAR_NEW_SURPRISE_CARD_LIMIT
			WHEN VAR_NEW_SURPRISE_CARD_LIMIT - (VAR_OLD_SURPRISE_CARD_LIMIT - SURPRISE_CARDS_REMAINING) < 0 THEN 0
			ELSE VAR_NEW_SURPRISE_CARD_LIMIT - (VAR_OLD_SURPRISE_CARD_LIMIT - SURPRISE_CARDS_REMAINING)
		END
...
DELIMITER //
CREATE PROCEDURE SP_SET_WILD_CARD_LIMIT (
    IN VAR_LOBBY_ID UUID,
    IN VAR_NEW_WILD_CARD_LIMIT INT
)
BEGIN
    DECLARE VAR_OLD_WILD_CARD_LIMIT INT;

    SELECT WILD_CARD_LIMIT
    INTO VAR_OLD_WILD_CARD_LIMIT
    FROM LOBBY
    WHERE ID = VAR_LOBBY_ID;
    
    UPDATE LOBBY
    SET WILD_CARD_LIMIT = VAR_NEW_WILD_CARD_LIMIT
    WHERE ID = VAR_LOBBY_ID;
    
    UPDATE PLAYER 
    SET WILD_CARDS_REMAINING = 
		CASE 
			WHEN VAR_NEW_WILD_CARD_LIMIT = -1 THEN -1
			WHEN VAR_NEW_WILD_CARD_LIMIT = 0 THEN 0
			WHEN VAR_OLD_WILD_CARD_LIMIT = -1 THEN VAR_NEW_WILD_CARD_LIMIT
            WHEN VAR_OLD_WILD_CARD_LIMIT = 0 THEN VAR_NEW_WILD_CARD_LIMIT
			WHEN VAR_NEW_WILD_CARD_LIMIT - (VAR_OLD_WILD_CARD_LIMIT - WILD_CARDS_REMAINING) < 0 THEN 0
			ELSE VAR_NEW_WILD_CARD_LIMIT - (VAR_OLD_WILD_CARD_LIMIT - WILD_CARDS_REMAINING)
		END
    WHERE LOBBY_ID = VAR_LOBBY_ID;
END;
//
...
CREATE PROCEDURE SP_GET_DECK_ACCESS (
    IN VAR_USER_ID UUID,
    IN VAR_ACCESS_USE_CASE VARCHAR(10)
)
BEGIN
    ...
    IF VAR_USER_IS_ADMIN = 1 THEN
        ...
        WHERE NOT IS_HIDDEN
        ORDER BY D.NAME ASC;
    ELSEIF VAR_ACCESS_USE_CASE = "LOBBY" THEN
        SELECT DISTINCT
            D.ID,
            D.NAME      
        FROM DECK AS D
...
(
    ...
    SURPRISE_CARDS_REMAINING INT NOT NULL DEFAULT -1,
    WILD_CARDS_REMAINING INT NOT NULL DEFAULT 0, 
...
BEGIN
    ...
    DECLARE VAR_SURPRISE_CARD_LIMIT INT;
    DECLARE VAR_WILD_CARD_LIMIT INT;
    
    SELECT SURPRISE_CARD_LIMIT, WILD_CARD_LIMIT
    INTO VAR_SURPRISE_CARD_LIMIT, VAR_WILD_CARD_LIMIT
    FROM LOBBY
    WHERE ID = VAR_LOBBY_ID;
    	
    INSERT IGNORE INTO PLAYER (ID, LOBBY_ID, USER_ID, SURPRISE_CARDS_REMAINING, WILD_CARDS_REMAINING)
    VALUES (VAR_PLAYER_ID, VAR_LOBBY_ID, VAR_USER_ID, VAR_SURPRISE_CARD_LIMIT, VAR_WILD_CARD_LIMIT);
...
BEGIN
    ...
    INSERT INTO BOARD (LOBBY_ID, PLAYER_ID, CARD_ID, IS_SURPRISE, IS_WILD)
    VALUES (VAR_LOBBY_ID, VAR_PLAYER_ID, VAR_CARD_ID, VAR_IS_SURPRISE, VAR_IS_WILD);

    INSERT INTO LOG_PLAY (PLAYER_USER_ID, JUDGE_USER_ID, CARD_ID, IS_SURPRISE, IS_WILD)
    VALUES(VAR_PLAYER_USER_ID, VAR_JUDGE_USER_ID, VAR_CARD_ID, VAR_IS_SURPRISE, VAR_IS_WILD);
...
BEGIN
    ...
    UPDATE PLAYER 
    SET SURPRISE_CARDS_REMAINING = SURPRISE_CARDS_REMAINING - 1
    WHERE ID = VAR_PLAYER_ID
		AND SURPRISE_CARDS_REMAINING > 0;
    
	INSERT INTO LOG_DRAW (PLAYER_USER_ID, CARD_ID, IS_SURPRISE)
    VALUES(VAR_PLAYER_USER_ID, VAR_SURPRISE_CARD_ID, 1);

    CALL SP_PLAY_CARD(VAR_PLAYER_ID, VAR_SURPRISE_CARD_ID, 1, 0);
...
CREATE PROCEDURE SP_PLAY_WILD_CARD (
    IN VAR_PLAYER_ID UUID,
    IN VAR_WILD_CARD_TEXT VARCHAR(255)
)
BEGIN
    DECLARE VAR_LOBBY_ID UUID;
    DECLARE VAR_WILD_CARD_ID UUID;
    DECLARE VAR_PLAYER_USER_ID UUID;
    DECLARE VAR_WILD_CARD_DECK_ID UUID;
    DECLARE VAR_NEW_WILD_CARD_ID UUID;
    DECLARE VAR_RANDOM_PW UUID;
    
    SELECT UUID()
    INTO VAR_RANDOM_PW;
    
	INSERT IGNORE INTO DECK (NAME, IS_HIDDEN, PASSWORD_HASH)
    VALUES ('WILD', 1, VAR_RANDOM_PW);
    
    SELECT ID
    INTO VAR_WILD_CARD_DECK_ID
    FROM DECK
    WHERE Name = 'WILD'
		AND IS_HIDDEN;
        
	SELECT UUID()
    INTO VAR_NEW_WILD_CARD_ID;
        
	INSERT INTO CARD(ID, DECK_ID, CATEGORY, TEXT)
    VALUES(VAR_NEW_WILD_CARD_ID, VAR_WILD_CARD_DECK_ID, 'PLAYER', VAR_WILD_CARD_TEXT);
    
    SELECT ID
    INTO VAR_WILD_CARD_ID
    FROM CARD 
    WHERE ID = VAR_NEW_WILD_CARD_ID;
    ...
    UPDATE PLAYER 
    SET WILD_CARDS_REMAINING = WILD_CARDS_REMAINING - 1
    WHERE ID = VAR_PLAYER_ID
		AND WILD_CARDS_REMAINING > 0;
        
        
	CALL SP_PLAY_CARD(VAR_PLAYER_ID, VAR_WILD_CARD_ID, 0, 1);
...
BEGIN
	...
	SELECT IS_SURPRISE, IS_WILD
    INTO VAR_IS_SURPRISE, VAR_IS_WILD
    FROM BOARD 
	WHERE LOBBY_ID = VAR_LOBBY_ID
		AND CARD_ID = VAR_CARD_ID;
	...
	INSERT INTO LOG_WIN (PLAYER_USER_ID, JUDGE_USER_ID, CARD_ID, IS_SURPRISE, IS_WILD)
	VALUES(VAR_PLAYER_USER_ID, VAR_JUDGE_USER_ID, VAR_CARD_ID, VAR_IS_SURPRISE, VAR_IS_WILD);
